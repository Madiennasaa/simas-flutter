  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        boxShadow: [
          BoxShadow(color: AppColors.neuShadowDark, offset: const Offset(2, 2), blurRadius: blur),
          BoxShadow(color: AppColors.neuShadowLight, offset: const Offset(-2, 2), blurRadius: blur),
        ],
      ),
      child: child,
    );
...
class NeuInnerShadow extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return CustomPaint(
      foregroundPainter: _InnerShadowPainter(color: color, offset: offset, blur: blur),
      child: child,
    );
  }
}
class _InnerShadowPainter extends CustomPainter {
  ...
  final Offset offset;
  final double blur;
  _InnerShadowPainter({required this.color, required this.offset, required this.blur});
  @override
  ...
  bool shouldRepaint(covariant _InnerShadowPainter oldDelegate) {
    return oldDelegate.color != color || oldDelegate.offset != offset || oldDelegate.blur != blur;
  }
}
  ...
  Widget build(BuildContext context) {
    return ClipRRect(
      ...
      child: NeuInnerShadow(
        color: const Color(0xFFF7F7F7).withOpacity(0.25),
